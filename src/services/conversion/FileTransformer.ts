...
export class FileTransformer {
      ...
      for (let i = 0; i < totalFiles; i += batchSize) {
        ...
        batchResults.forEach(batchResult => {
          if (batchResult.modified) {
            ...
            details.push(`Transformations in file: ${batchResult.fileName}\n${batchResult.transformations.join('\n')}`);
          }
        });
      }
   ...
   */
  private async transformFile(file: File, options: ConversionOptions): Promise<{
    fileName: string;
    modified: boolean;
  ...
  async replaceComponents(): Promise<{
    ...
    try {
      ...
      for (const file of this.files) {
        ...
        try {
          ...
          for (const componentType of componentTypes) {
            // Skip if component not used in file
            if (this.isComponentUsed(content, componentType)) {
              ...
              if (transformResult.code !== content) {
                result.replacedComponents.push({
                  ...
                  count: this.countComponentOccurrences(content, componentType)
                });
          ...
          this.errorCollector.addError({
            ...
            severity: 'warning',
            message: `Error replacing components in ${file.name}: ${error instanceof Error ? error.message : String(error)}`,
            file: file.name
          });
        }
      }
      ...
      this.errorCollector.addError({
        ...
        severity: 'warning',
        message: `Component replacement process failed: ${error instanceof Error ? error.message : String(error)}`
      });
  ...
  /**
   * Check if a component type is used in the file content
   */
  private isComponentUsed(content: string, componentType: string): boolean {
    switch (componentType.toLowerCase()) {
      case 'image':
        return content.includes("from 'next/image'") || 
               content.includes('from "next/image"');
      case 'link':
        return content.includes("from 'next/link'") || 
               content.includes('from "next/link"');
      case 'head':
        return content.includes("from 'next/head'") || 
               content.includes('from "next/head"');
      case 'script':
        return content.includes("from 'next/script'") || 
               content.includes('from "next/script"');
      case 'dynamic':
        return content.includes("from 'next/dynamic'") || 
  ...
  /**
   * Count component occurrences in content
   */
  private countComponentOccurrences(content: string, componentType: string): number {
    ...
    switch (componentType.toLowerCase()) {
      ...
      case 'script':
        ...
        break;
      case 'dynamic':
        // Dynamic is a function, not a component
        return (content.match(/dynamic\(/g) || []).length;
      default:
        return 0;
    }
    
    // Count JSX component usage
